# DataBridge Global - Extracted from CAADataBridge/global.R
...
# ============================================================================
# Preferred column order for CSV export. Contains both CValR-renamed names
# (standard mode with restrict) and internal names (dest mode / no restrict).
# reorder_export_columns() uses intersect() so only columns present in the
# dataframe are matched — works for both modes automatically.

EXPORT_COLUMN_ORDER <- c(
  # Group 1 - IDs
  "MLS #", "ListingId", "Listing Key Numeric", "ListingKey",
  # Group 2 - Address
  "Address",
  # Group 3 - Listed
  "On Market Date", "DateListed", "List Price", "PriceListed",
  # Group 4 - Sold
  "Sold Date", "DateSold", "Sold Price", "PriceSold",
  # Group 5 - Other dates/prices
  "Purchase Contract Date", "DateContract",
  "Off Market Date", "DateOffMarket",
  "Title/Ownership Change Date", "DateTitleChange",
  "Original List Price", "PriceOrigList",
  # Group 6 - Status
  "Status",
  # Group 7 - Location
  "City", "Postal Code", "PostalCode",
  "Neighbourhood", "SubdivisionName",
  "Minor/Sub Area", "SubArea",
  "Major Area/Region", "MajorArea",
  # Group 8 - Property
  "Property Sub Type", "PropertySubType",
  "Year Built", "YearBuilt",
  "Style", "Condition",
  "Beds", "BA", "PB",
  "SqFt", "Basement", "Garage",
  # Group 9 - Lot/Land
  "Lot Acres", "LotAcres",
  "Lot Dimensions", "LotDimensions",
  "Zoning",
  # Group 10 - Features
  "View", "Pool", "Waterfront",
  "Sewer", "Water Source", "WaterSource",
  "Green Energy", "GreenEnergy",
  "Strata Fees", "StrataFees",
  # Group 11 - Legal
  "Parcel ID", "ParcelId",
  "Tax Legal Description", "TaxLegalDesc",
  # Group 12 - Remarks
  "REALTOR Remarks", "RealtorRemarks",
  "Public Remarks", "PublicRemarks",
  # Group 13 - Derived metrics
  "PricePerSqFt", "PricePerLotSF", "SaleQtr"
)

reorder_export_columns <- function(df) {
  known <- intersect(EXPORT_COLUMN_ORDER, names(df))
  remaining <- setdiff(names(df), known)
  df[, c(known, remaining), drop = FALSE]
}
