# DataBridge Server - Extracted from CAADataBridge/server.R
# Lines 287+ (main app logic, excluding auth/chooser handlers)
# Wrapped in databridge_server_logic() for module use

#' DataBridge Server Logic
#' @param input Shiny input
#' @param output Shiny output
#' @param session Shiny session
#' @return List with mapped_data and field_map reactives
databridge_server_logic <- function(input, output, session) {
  ns <- session$ns

  # Namespaced version of id_for for use in renderUI (creating inputs).
  # Regular id_for is used for input[[]] reads (Shiny auto-strips namespace).
  ns_id_for <- function(tgt, dest_mode = FALSE) {
    ns(id_for(tgt, dest_mode))
  }
    ...
    workflow_wizard_answered <- reactiveVal(TRUE)  # No wizard in CAACollaborative - Home page is the entry point
    user_has_profile <- reactiveVal(FALSE)  # Default to no profile in CAACollaborative (wizard disabled)
    ...
    # Workflow wizard disabled in CAACollaborative - Home page serves as entry point
    # Users navigate to Column Mapper tab when ready
    # observe({
    #   if (!workflow_wizard_answered()) {
    #     showModal(...)
    ...
    observeEvent(input$wizard_yes, {
      ...
      # Show brief guidance then auto-open the file explorer for profile upload
      showNotification(
        HTML("<strong>Select your mapping profile</strong><br/>After loading, upload your CSV data to auto-apply mappings."),
        type = "message",
        duration = 8,
        closeButton = TRUE
      )
  
      # Trigger click on the file input to open file explorer directly
      shinyjs::delay(500, shinyjs::click("upload_mapping"))
    })
    ...
    observeEvent(input$upload_csv, {
      ...
      tryCatch({
        ...
        if (ext %in% c("xlsx", "xls")) {
          ...
          # Use readr::read_csv() for RFC 4180 compliant parsing.
          # Base R read.csv() can't handle embedded newlines in quoted fields
          # (common in MLS Remarks), causing rows to split and columns to shift.
          # readr handles these correctly and auto-detects encoding.
          uploaded <- tryCatch({
            df <- suppressWarnings(as.data.frame(
              read_csv(fpath, show_col_types = FALSE, locale = locale(encoding = "latin1"),
                       name_repair = "minimal"),
              check.names = FALSE
            ))
            message(sprintf("CSV read with readr (latin1): %d rows, %d columns", nrow(df), ncol(df)))
            df
          }, error = function(e) {
            message(sprintf("readr latin1 failed (%s), trying UTF-8...", e$message))
            tryCatch({
              as.data.frame(
                read_csv(fpath, show_col_types = FALSE, locale = locale(encoding = "UTF-8"),
                         name_repair = "minimal"),
                check.names = FALSE
              )
            }, error = function(e2) {
              message(sprintf("readr UTF-8 failed (%s), falling back to base read.csv...", e2$message))
    ...
    observeEvent(id_analysis_result(), {
      ...
      if (analysis$needs_prompt && nrow(analysis$candidates) >= 2) {
        ...
        showModal(modalDialog(
          ...
          footer = tagList(
            actionButton(ns("id_choice_confirm"), "Use Selected Column", class = "btn-primary"),
            actionButton(ns("id_choice_skip"), "Skip (use auto-suggestion)", class = "btn-default")
          ),
          easyClose = FALSE,
          size = "m"
        ))
    ...
    output$workflow_sidebar <- renderUI({
      ...
      if (is.null(user_choice)) {
        ...
        tagList(
          ...
          uiOutput(ns("profile_upload_ui")),
          hr(),
          h4("2. Data Upload"),
          fileInput(ns("upload_csv"), "Upload Your CSV/Excel", accept = c(".csv", ".xlsx", ".xls")),
          ...
          tags$details(
            ...
            verbatimTextOutput(ns("current_mappings"))
          )
        )
      } else {
      ...
      tags$div(
        ...
        tags$div(
          ...
          fluidRow(
            ...
            column(3,
              selectInput(ns("mapping_format"), "Export Format:",
          ...
          fluidRow(
            ...
            column(4,
              tags$div(
                ...
                actionButton(ns("launch_decision_trees"),
    ...
    # In CAACollaborative, Decision Trees is a tab in the same app.
    # Navigate to it via the host navbar instead of launching a separate process.
    launch_decision_trees_handler <- function() {
      # Switch to the Decision Trees tab in the host navbar
      # Use session$rootScope()$ns or parent session to reach the host-level navbar
      nav_select("main_navbar", selected = "dt_tab", session = session$userData$root_session %||% session)
      showNotification("Switched to Decision Trees tab. Mapped data is available.", type = "message", duration = 5)
    }
    ...
    output$profile_upload_ui <- renderUI({
      ...
      if (is.null(current_dataset())) {
        ...
        tagList(
          fileInput(ns("upload_mapping"), "Load Mapping Profile",
   ...
   # Get mappable columns (Class 1, 2 & 3) for the mapping UI
          ...
          tags$div(
            ...
            lapply(filtered_dest_headers, function(dest_col) {
              ...
              if (isTRUE(derived_info$is_derivable) && derived_info$metric_name == "SaleQtr") {
                ...
                if (has_selection && nzchar(selected_primary)) {
                  ...
                  suggestion_buttons <- lapply(found_date_cols, function(col) {
                    actionLink(
                      inputId = ns(paste0("add_derived_dest_", safe_dest, "_", make.names(col))),
                      label = col,
              ...
              sqft_derive_ui_dest <- if (!is.null(meta) && !is.null(meta$derive_option) &&
                ...
                uiOutput(ns("sqft_derive_panel_dest"))
              } else {
                NULL
              }
              ...
              separator_input <- if (is_multi && !isTRUE(meta$combine == "sum")) {
                div(
                  ...
                  tagList(
                    textInput(
                      inputId = ns(paste0("sep_dest_", make.names(dest_col))),
                      label = "Separator",
              ...
              address_suggest_ui_dest <- if (is_multi && grepl("address", tolower(dest_col))) {
                ...
                if (length(suggested_additions) > 0) {
                  suggestion_buttons <- lapply(names(suggested_additions), function(name) {
                    ...
                    safe_dest <- make.names(dest_col)
                    actionLink(
                      inputId = ns(paste0("add_addr_dest_", safe_dest, "_", make.names(col))),
                      label = col,
              ...
              preview_button <- actionButton(
                inputId = ns(paste0("preview_dest_", make.names(dest_col))),
                label = icon("eye"),
              ...
              class3_review_ui <- if (is_class3_source && review_state == "pending" && (has_selection || nzchar(suggested))) {
                ...
                tags$div(
                  ...
                  tags$div(
                    ...
                    radioButtons(
                      inputId = ns(paste0("class3_choice_", safe_dest)),
                      label = NULL,
                      choices = c(
                  ...
                  tags$div(
                    ...
                    actionButton(
                      inputId = ns(paste0("accept_class3_", safe_dest)),
                      label = "Accept",
                      class = "btn-success btn-sm"
                    ),
                    actionButton(
                      inputId = ns(paste0("skip_class3_", safe_dest)),
                      label = "Skip",
                ...
                tags$div(
                  ...
                  actionLink(
                    inputId = ns(paste0("reconsider_class3_", safe_dest)),
                    label = "Reconsider",
              ...
              exclusion_info_dest <- if (num_excluded > 0) {
                tags$div(
                  ...
                  tags$small(
                    ...
                    actionLink(
                      inputId = ns(paste0("manage_exclusions_dest_", make.names(dest_col))),
              ...
              exclude_button_dest <- actionButton(
                inputId = ns(paste0("exclude_dest_", make.names(dest_col))),
          ...
          tags$div(
            ...
            lapply(target_names, function(tgt) {
                ...
                # Build the JavaScript render function with substituted values
                  ...
                  address_suggest_ui <- if (identical(tgt, "Address") && has_selection) {
                    ...
                    if (length(suggested_additions) > 0) {
                      suggestion_buttons <- lapply(names(suggested_additions), function(name) {
                        col <- suggested_additions[[name]]
                        actionLink(
                          inputId = ns(paste0("add_address_", make.names(col))),
                          label = col,
                          style = "display: inline-block; margin: 2px 4px 2px 0; padding: 2px 8px; background: #e3f2fd; border-radius: 12px; font-size: 11px; color: #1565c0; text-decoration: none;",
                  ...
                  exclusion_info <- if (num_excluded > 0) {
                    tags$div(
                      ...
                      tags$small(
                        ...
                        actionLink(
                          inputId = ns(paste0("manage_exclusions_", tgt)),
                          label = "Review/Restore",
                          style = "font-size: 11px; color: #856404; text-decoration: underline;"
                        )
                      )
                  ...
                  preview_button <- actionButton(
                    inputId = ns(paste0("preview_", tgt)),
                    label = icon("eye"),
                    class = "btn btn-sm btn-info preview-btn",
                    title = "Preview selected column data",
                  ...
                  exclude_button <- actionButton(
                    inputId = ns(paste0("exclude_", tgt)),
                    label = if (num_excluded > 0)
                      sprintf("None of These Match (%d hidden)", num_excluded)
                    else
                      "None of These Match",
                  ...
                  class3_review_ui_cvalr <- if (is_class3_source_cvalr && review_state_cvalr == "pending" && (has_selection || nzchar(suggested_primary))) {
                    ...
                    tags$div(
                      ...
                      tags$div(
                        ...
                        tags$strong("Accept mapping as:", style = "font-size: 12px;"),
                        radioButtons(
                          inputId = ns(paste0("class3_choice_", safe_tgt)),
                          label = NULL,
                          choices = c(
                            "Keep Class 3 (Marginal)" = "3",
                            "Upgrade to Class 2 (Supplemental)" = "2",
                      ...
                      tags$div(
                        style = "display: flex; gap: 8px;",
                        actionButton(
                          inputId = ns(paste0("accept_class3_", safe_tgt)),
                          label = "Accept",
                          class = "btn-success btn-sm"
                        ),
                        actionButton(
                          inputId = ns(paste0("skip_class3_", safe_tgt)),
                          label = "Skip",
                          class = "btn-default btn-sm"
                        )
                      )
                    ...
                    tags$div(
                      ...
                      tags$span("Skipped suggestion: ", source_col_display),
                      actionLink(
                        inputId = ns(paste0("reconsider_class3_", safe_tgt)),
                        label = "Reconsider",
                        style = "margin-left: 10px;"
                      )
                    )
        ...
        if (!length(group_panels)) {
          ...
          hint_message <- if (search_active) {
            tagList(
              ...
              actionButton(ns("clear_mapping_search"), "Clear Search",
                          class = "btn-default btn-sm",
        ...
        local({
          ...
          checkbox_list <- lapply(excluded_cols, function(col) {
            tags$div(
              style = "padding: 4px 0;",
              checkboxInput(
                inputId = ns(paste0("restore_dest_", local_safe, "_", make.names(col))),
          ...
          showModal(modalDialog(
            ...
            footer = tagList(
              actionButton(ns("restore_selected"), "Restore Selected", class = "btn-primary"),
              actionButton(ns("restore_all"), "Restore All", class = "btn-warning"),
              modalButton("Cancel")
            ),
            easyClose = TRUE,
            size = "m"
      ...
      observeEvent(input[[manage_link_id]], {
        ...
        checkbox_list <- lapply(excluded_cols, function(col) {
          tags$div(
            style = "padding: 4px 0;",
            checkboxInput(
              inputId = ns(paste0("restore_", target_name, "_", make.names(col))),
              label = tags$span(style = "color: #212529; font-weight: normal;", col),
              value = FALSE
            )
          )
          ...
          observeEvent(input[[local_edit_id]], {
            ...
            showModal(modalDialog(
              ...
              tags$div(
                ...
                textAreaInput(
                  inputId = ns(paste0("synonyms_input_std_", make.names(local_tgt))),
          ...
          observeEvent(input[[local_edit_id]], {
            ...
            showModal(modalDialog(
              ...
              tags$div(
                ...
                tags$p("Enter synonyms separated by commas. These help the auto-mapper identify this column in future datasets."),
                textAreaInput(
                  inputId = ns(paste0("synonyms_input_", make.names(local_dest_col))),
                  label = "Synonyms (comma-separated)",
                  value = syn_text,
                  rows = 4,
                  width = "100%",
    ...
    observeEvent(input$apply_mappings, {
      ...
      if (isTRUE(input$restrict_to_destination_only)) {
        ...
        # Apply CValR field rename (from shared/cvalr_field_map.R)
        cvalr_result <- apply_cvalr_rename(final_data)
        final_data <- cvalr_result$data
        message(sprintf("RESTRICT RENAME: %d columns renamed to CValR dest template names", cvalr_result$renamed_count))
        message(sprintf("RESTRICT FINAL COLUMNS: %s", paste(names(final_data), collapse = ", ")))
      }
  
      # Reorder columns to preferred export order
      final_data <- reorder_export_columns(final_data)
    ...
    observe({
      ...
      if (length(new_class3) > 0) {
        ...
        showModal(modalDialog(
          ...
          footer = tagList(
            actionButton(ns("upgrade_class3_yes"), "Yes, upgrade to Class 2", class = "btn-primary"),
            actionButton(ns("upgrade_class3_no"), "No, keep as Class 3", class = "btn-default"),
      ...
      content = function(file) {
        tryCatch({
          ...
          if (is.null(data_to_export)) {
            stop("No mapped data available. Please apply mappings first.")
          }
  ...
  # Return reactives for host app integration
  return(list(
    mapped_data = reactive({ if (exists("mapped_data")) mapped_data() else NULL }),
    field_map = reactive({ if (exists("value_source_tracker")) value_source_tracker() else list() })
  ))
} # end databridge_server_logic
